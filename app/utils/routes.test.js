...
describe("Test findAll endpoint", () => {
  ...
  it("Should get all products paginated with varying page", async () => {
    const resPageOne = await request(app).get("/api/products?page=1");
    ...
    const resPageTwo = await request(app).get("/api/products?page=2");
    ...
    const resPageThree = await request(app).get("/api/products?page=3");
